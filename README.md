...
The webpack configuration uses `source-map` as devtool in this environemnt. It creates high-quality source maps. Besides, the following three plugins were also used to:
- `mini-css-extract-plugin` - extract CSS into separate files,
- `css-minimizer-webpack-plugin` - optimize and minify CSS,
- `terser-webpack-plugin` - optimize and minify JavaScript.

Use the command below to run Webpack and bundle your application.
